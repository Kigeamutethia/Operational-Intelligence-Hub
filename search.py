...
st.title("📊 Enterprise Data Catalogue (Purview-Lite)")
    ...
    for root, dirs, files in os.walk(BASE_DIR):
        for f in files:
            ...
            try:
                ...
                """, (
                    f, path, ext, size_kb, tag,
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                ))

                if df is not None:
                    profile = extract_profile(path)[1]
...
query = st.sidebar.text_input("Search files / columns")
